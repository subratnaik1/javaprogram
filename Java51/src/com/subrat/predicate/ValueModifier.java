...
class ValueModifier{
	public static void main(String[]args) {
		int ov=Integer.parseInt(IO.readln("Original value"));
		int inc=Integer.parseInt(IO.readln("increment by"));
		Consumer<Integer>doubleValue=d->IO.println("After doubling the value: "+d*2);
		Consumer<Integer>incrementBy=i->IO.println("After incrementing the value by: "+i+inc);
		Consumer<Integer>squareValue=sq->IO.println("After squaring the value: "+sq*sq);
		
		ValueModifier.modifyValue(ov,doubleValue);
		ValueModifier.modifyValue(ov, incrementBy);
		ValueModifier.modifyValue(ov, squareValue);
	}
...
//The program should include three consumers: doubleValue, incrementBy, and squareValue, each with different modification operations.
//Your task is to implement the modifyValue method such that it accepts an integer value and a consumer as parameters,
...
//Expected Output :
//Original value: -8
//After doubling the value: -16import java.util.Scanner;
